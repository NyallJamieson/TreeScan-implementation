...
# final TreeScan count file order: code, date, n
count_file_df <- input_file[, c("code", "date", "n")]

# inspect
head(count_file_df)

count_file_path <- paste0(parent_dir, "/data/Analysis_Count_File.txt")

write.table(
  count_file_df,
  file = count_file_path,
  sep = "\t",
  row.names = FALSE,
  col.names = TRUE,
  quote = FALSE,
...
write.table(input_file, paste0(parent_dir, "/data/Analysis_Count_File.txt"), quote=FALSE, row.names=FALSE, sep="\t", eol="\r\n")
write.csv(input_file, paste0(parent_dir, "/data/Analysis_Count_File.csv"), row.names = FALSE, quote = FALSE)
