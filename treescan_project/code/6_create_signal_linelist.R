...
for (lag in initial_lags){
  ...
  if (length(Nodes) > 0){
    ...
    for(i in 1:nrow(TS_Results_today))
    {
      ...
      node_raw <- TS_Results_today$Node.Identifier[i]
      
      # node_raw <- gsub("\\.", "", node_raw)
      # node_codes <- unlist(strsplit(node_raw, "\\|"))
      # node_codes <- gsub("^(0-|1-|2-)", "", node_codes)
      # node_codes <- unique(trimws(node_codes))
      # node_codes <- node_codes[node_codes != ""]
      
      # clean node codes (KEEP dots)
      node_codes <- TS_Results_today$Node.Identifier[i]
      node_codes <- unlist(strsplit(node_codes, "\\|"))
      node_codes <- gsub("^(0-|1-|2-)", "", node_codes)
      node_codes <- trimws(node_codes)
      node_codes <- node_codes[node_codes != ""]
      ...
      if(nrow(temp1)>0)
      {
        ...
        if (grepl("^1\\-", TS_Results_today$Node.Identifier[i])) {
        ...
        plot(
          ...
          xlab = "CDC_Week",
          ylab = gsub("2\\-", "", paste(TS_Results_today$Node.Identifier[i])),
          xaxt = "n"
        )
        ...
        if (nrow(modzcta_plot1) > 0) {
          ...
          p1 <- ggplot(modzcta1) +
            ...
            theme_minimal() +
            labs(title = paste("MODZCTA Choropleth Map for", TS_Results_today$Node.Identifier[i], "baseline"))
        } else {
          p1 <- ggplot(modzcta1) +
            geom_sf(aes(fill = pct), color = "white", size = 0.1) +
            scale_fill_viridis_c(
        ...
        saveWorkbook(
          wb,
          paste0(
            parent_dir, "/signal_interpretation/", END_DATE, "_lag", lag, "_",
            gsub("\\|", "_", gsub("2\\-", "", TS_Results_today$Node.Identifier[i])),
            ".xlsx"
          ),
          overwrite = TRUE
        )
