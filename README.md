...
### Alternative: Running Individual Steps

If you need to run individual steps separately (e.g., only reprocessing count data), you can run sections of `run_all.R` individually, or source the specific script files:

<details>
<summary><b>macOS</b></summary>

```r
# Download ICD-10 files only
source(here::here("code", "0_Download_ICD10_Files.R"))
download_icd10_files(year = 2026)

# Create tree file only
source(here::here("code", "1_Create_Tree_File.R"))
create_tree_file(year = 2026, debug_mode = FALSE)

# Process ED count data only
...
run_treescan(
  ...
  ph_dept = "NYC-DHMH",
  data_file = "synthetic"
  # Default path: /Applications/TreeScan.app/Contents/app/treescan
)
```
...
run_treescan(
  ...
  ph_dept = "NYC-DHMH",
  data_file = "synthetic",
  treescan_path = "C:/Program Files/TreeScan/treescan.exe"  # Adjust if different
)
```
...
run_treescan(
  ...
  ph_dept = "NYC-DHMH",
  data_file = "synthetic",
  treescan_path = "/opt/treescan.2.3/treescan64"  # Adjust to your installation
)
```
