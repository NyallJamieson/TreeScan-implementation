...
# set location of "Preparing TreeScan Input Files" folder
directory <-"//nasprgshare220/Share/DIS/BCD/COMDISshared/Analyst_of_the_week/Asyndromic/Manuscript/Draft/Supplement/Preparing TreeScan Input Files/"
...
treefile = paste0(directory,"TreeScan input files/Final_Tree_File_",year,".csv")
countfile = paste0(directory,"TreeScan input files/Analysis_Count_File.txt")
date_range = paste0("[",format(min(input_file$date), "%Y/%m/%d"),",",format(max(input_file$date), "%Y/%m/%d"),"]")
resultsfile = paste0(directory,"TreeScan output files/Results_",format(max(input_file$date), "%Y%m%d"),".txt")
paramfile = paste0("TreeScan input files/Param_File.prm")
makeprm_2_3_0(treefile, countfile, date_range, resultsfile, paramfile)

#Run in batch mode
system(paste("/opt/treescan.2.3/treescan64" ,' ', paramfile, sep=''))
